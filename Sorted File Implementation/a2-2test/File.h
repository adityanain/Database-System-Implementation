...
public:
	...
	bool isEmpty() const{ return numRecs == 0;}
...
public:
	...
	//index of the last page in the file
	off_t GetLastIndex() const { return curLength - 2; }

	bool isEmpty() const { return curLength == 0; }

	void GetLastPage(Page* p_LastPage){
		if(isEmpty()) 
			p_LastPage = NULL;
		else 
		    GetPage(p_LastPage, GetLastIndex());
	}
