                                 ...
                                 OrderMaker& queryorder, OrderMaker& cnforder)
{
  ...
  for(size_t i = 0; i < sortOrder.numAtts; ++i){
  	int att  = sortOrder.whichAtts[i];
  	Type type = sortOrder.whichTypes[i];
    int ii = findAttrIn(att, query);
    if (ii >= 0) {     // found this attribute in query CNF
      ...
      //UNPACK2(cnforder.whichAtts[cnforder.numAtts], cnforder.whichTypes[cnforder.numAtts], ii, type);
    	cnforder.whichAtts[cnforder.numAtts] = ii;
    	cnforder.whichTypes[cnforder.numAtts] = type;
      ++queryorder.numAtts;
      ++cnforder.numAtts;
    }else 
    	return;   // don't search further
  }
}
...
int OrderMaker::findAttrIn(int att, const CNF& query) {
  ...
  for(size_t i = 0; i < query.numAnds; ++i){
  	const Comparison* clause = query.orList[i];
  	int orLen = query.orLens[i];
    if (orLen == 1) {
      const Comparison& cmp = clause[0];
      if (cmp.op == Equals && (((cmp.whichAtt1 == att) && (cmp.operand2 == Literal))
...
int CNF::GetOrder (OrderMaker &left, OrderMaker &right) {
    ...
    for (int i = 0; i < numAnds; i++)
    {
        ...
        {
            // get type and column position info
            left.whichAtts[left.numAtts] = orList[i][0].whichAtt2;
            left.whichTypes[left.numAtts] = orList[i][0].attType;
            // get type and the position info in CNF
            right.whichAtts[right.numAtts] = orList[i][0].whichAtt1;
            right.whichTypes[right.numAtts] = orList[i][0].attType;
        }
        else
...
OrderMaker* CNF::bSearchQueryMaker(OrderMaker& sortOrder){
	OrderMaker cnfOrder;
	OrderMaker mSortOrder = sortOrder;
	GetOrder(cnfOrder, mSortOrder);
	OrderMaker *pOrderedQuery = new OrderMaker();
	...
	for (int i = 0; i < sortOrder.numAtts; i++)
	{
		...
		for(int j = 0; j < cnfOrder.numAtts; j++)
		{
			if((sortOrder.whichAtts[i] == cnfOrder.whichAtts[j]) && 
				(sortOrder.whichTypes[i] == cnfOrder.whichTypes[j])){

				cout<<"Attributes matched at i : "<<i<<" j : "<<j<<endl;
				isMatching = true;
				pOrderedQuery->whichAtts[j] = mSortOrder.whichAtts[j];
				pOrderedQuery->whichTypes[j] = mSortOrder.whichTypes[j];
				pOrderedQuery->numAtts++;
				break;
			}
		}
		if(!isMatching)
	...
	if(pOrderedQuery->numAtts == 0){
		delete pOrderedQuery;
		pOrderedQuery = NULL;
	}
	return pOrderedQuery;
}
